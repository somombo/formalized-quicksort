...
-- macro "myomega" : tactic => `(tactic| admit)
...
where
  @[specialize] loop (pivot : α) (arr : Vector α n) (i j : Nat) (p q : Nat)
      ...
      /- (haqltp : ¬lt arr[q + 1] pivot) -/
      : { x : Partition α n  // left < x.i' ∧ x.j' < right } :=

    let ⟨⟨j', (_ : j' < n)⟩, (_ : j' ≤ j)⟩  := loop.while_j' left right hr (by omega) pivot arr i j hjr (by omega)
      j (by omega) Nat.le.refl

    let ⟨⟨i', (_ : i' < n)⟩, (_ : i ≤ i'), (_ : i' ≤ q + 1)⟩ := loop.while_i' left (q + 1) (by omega) pivot arr i j' (by omega)
    ...
    if _ : i' < j' then
      ...
      -- have _ : q > 0 := by omega
      -- have _ : j' ≤ q := by omega
      -- have _ : j' < q + 1 := by omega
      if ha'ipiv : lt arr'[i'] pivot then
        if ha'pivj : lt pivot arr'[j'] then
          -- show : ¬lt arr'[q + 1] pivot
          loop pivot arr' (i' + 1) (j' - 1) p q (by omega) (by omega) (by omega) (by omega) (by omega) (by omega)
        else
          let arr'' := arr'.swap q j' (by omega) (by omega)
          -- show : ¬lt arr''[q] pivot
          loop pivot arr'' (i' + 1) (j' - 1) p (q - 1) (by omega) (by omega) (by omega) (by omega) (by omega) (by omega)
      else
        let arr'' := arr'.swap p i' (by omega) (by omega)
        if ha'pivj : lt pivot arr''[j'] then
          -- show : ¬lt arr''[q + 1] pivot
          loop pivot arr'' (i' + 1) (j' - 1) (p + 1) q (by omega) (by omega) (by omega) (by omega) (by omega) (by omega)
        else
          let arr''' := arr''.swap q j' (by omega) (by omega)
          -- show : ¬lt arr'''[q] pivot
          loop pivot arr''' (i' + 1) (j' - 1) (p + 1) (q - 1) (by omega) (by omega) (by omega) (by omega) (by omega) (by omega)
