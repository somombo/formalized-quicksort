...
theorem hoare.classic.while_i_elem_not_left_of_piv {arr : Vector α n} {harltp : ¬lt arr[right] pivot}
  : ¬lt arr[(hoare.classic.loop.while_i (lt := lt) left right hr pivot arr i j hjr harltp ival hii hxi).1.val] pivot := by
  induction ival, hii, hxi using hoare.classic.loop.while_i.induct (lt := lt) right hr pivot arr i j harltp with
...
theorem hoare.classic.while_j_elem_not_right_of_piv {arr : Vector α n} {halgep : ¬lt pivot arr[left]} :
  ¬lt pivot arr[(hoare.classic.loop.while_j (lt := lt) left right hr hl pivot arr i j hjr hli halgep   jval hxj hjj).1.val] := by
  induction jval, hxj, hjj using hoare.classic.loop.while_j.induct (lt := lt) left right hr hl pivot arr i j hjr halgep with
...
theorem hoare.classic.while_i_before_is_left {arr : Vector α n} {harltp : ¬lt arr[right] pivot} :
  RangeHas n (lt arr[·] pivot) ival (hoare.classic.loop.while_i (lt := lt) left right hr pivot arr i j hjr harltp ival hii hxi) := by
  induction ival, hii, hxi using hoare.classic.loop.while_i.induct (lt := lt) right hr pivot arr i j harltp with
  | case1 ival hii hxi hi hlt hir ih =>
    unfold hoare.classic.loop.while_i; simp [*]
...
theorem hoare.classic.while_j_before_is_right {arr : Vector α n} {halgep : ¬lt pivot arr[left]} :
  RangeHas n (lt pivot arr[·]) ((hoare.classic.loop.while_j (lt := lt) left right hr hl pivot arr i j hjr hli halgep   jval hxj hjj) + 1) (jval + 1) := by
  induction jval, hxj, hjj using hoare.classic.loop.while_j.induct (lt := lt) left right hr hl pivot arr i j hjr halgep with
  | case1 jval hxj hjj hj hlt hjl ih =>
    unfold hoare.classic.loop.while_j; simp [*]
...
theorem hoare.classic.loop.partition_bounds {left right : Nat} {hr : right < n} {hl : left < n} {pivot : α} {arr : Vector α n} {i j : Nat} {hli : left < i} {hij : i ≤ j + 1} {hjr : j < right} {halgep : ¬lt pivot arr[left]} {harltp : ¬lt arr[right] pivot} : (loop (lt := lt) left right hr hl pivot arr i j hli hij hjr halgep harltp).val.j' < (loop (lt := lt) left right hr hl pivot arr i j hli hij hjr halgep harltp).val.i' := by
...
theorem hoare.classic.loop.permStabilizing {left right : Nat} {hr : right < n} {hl : left < n} {pivot : α} {arr : Vector α n} {i j : Nat} {hli : left < i} {hij : i ≤ j + 1} {hjr : j < right} {halgep : ¬lt pivot arr[left]} {harltp : ¬lt arr[right] pivot} : PermStabilizing' i j (loop (lt := lt) left right hr hl pivot arr i j hli hij hjr halgep harltp).val.arr' arr := by
  induction arr, i, j, hli, hij, hjr, halgep, harltp using loop.induct left right hr hl pivot; all_goals unfold loop; simp [*]
  · case case1 arr i j hli hij hjr halgep harltp i' hi1' hi2' heq_i' j' hj' heq_j' hlt arr' halgep' harltp' ih =>
...
include lt_asymm in
theorem hoare.classic.loop.sorted {left right : Nat} {hr : right < n} {hl : left < n} {pivot : α} {arr : Vector α n} {i j : Nat} {hli : left < i} {hij : i ≤ j + 1} {hjr : j < right} {halgep : ¬lt pivot arr[left]} {harltp : ¬lt arr[right] pivot} : IsPartitioned (lt := lt) i j pivot (loop (lt := lt) left right hr hl pivot arr i j hli hij hjr halgep harltp).val := by
  induction arr, i, j, hli, hij, hjr, halgep, harltp using loop.induct left right hr hl pivot; all_goals unfold loop; simp [*]
  · case case1 arr i j hli hij hjr halgep harltp i' hi1' hi2' heq_i' j' hj' heq_j' hlt arr' halgep' harltp' ih =>
    constructor
    · refine RangeHas.trans ?_ ih.1
      intro k h1 h2
      rw [(loop.permStabilizing (lt := lt)).2.1 k h2]; exact
      if h : k = i' then by
        simp only [h, Fin.getElem_fin, arr.getElem_swap_left, show j' = _ from Subtype.ext_iff.1 heq_j'.symm]
        exact hoare.classic.while_j_elem_not_right_of_piv (lt := lt)
      else by
        ...
        apply hoare.classic.while_i_before_is_left (lt := lt) k h1 ((show i' = _ from Subtype.ext_iff.1 heq_i'.symm) ▸ h2)
    · refine RangeHas.trans ih.2 ?_
      intro k h1 h2
      rw [(loop.permStabilizing  (lt := lt)).2.2 k h1]; exact
      if h : k = j' then by
        simp only [h, Fin.getElem_fin, arr.getElem_swap_right, show i' = _ from Subtype.ext_iff.1 heq_i'.symm]
        exact hoare.classic.while_i_elem_not_left_of_piv (lt := lt)
      else by
        have := show (arr.swap i' j')[k] = _ by
          apply arr.getElem_swap_of_ne; all_goals first | simp only ; omega | omega
        simp only [this]
        ...
        apply hoare.classic.while_j_before_is_right (lt := lt) k ((show j' = _ from Subtype.ext_iff.1 heq_j'.symm) ▸ h1) h2
  ...
  · case case2 arr i j hli hij hjr halgep harltp i' hi1' hi2' heq_i' j' hj' heq_j' _ hlt' =>
    ...
    · intro k h1 h2
      ...
      apply hoare.classic.while_i_before_is_left (lt := lt) k h1 (this ▸ h2)
    · intro k h1 h2
      apply lt_asymm
      have := show j' = _ from Subtype.ext_iff.1 heq_j'.symm
      apply hoare.classic.while_j_before_is_right (lt := lt) k (this ▸ h1) h2
  ...
  · case case3 arr i j hli hij hjr halgep harltp p_ hi1' hi2' heq_i' p hj' heq_j' _hij' _hij' =>
    ...
    · apply RangeHas.append
      ...
      ·
        ...
        apply while_i_before_is_left (lt := lt) k h1 (this ▸ h2)
...
include lt_asymm in
private theorem hoare.classic.sortAt_sorted (as : Vector α n) (low high : Nat) (hlh : low ≤ high) (hn : high < n) : ¬ (lt (maybeSwap (lt := lt) as ⟨low, by omega⟩ ⟨high, by omega⟩)[high] (maybeSwap (lt := lt) as ⟨low, by omega⟩ ⟨high, by omega⟩)[low]) := by
...
-- ∀ {k_ : Nat} (_ : left ≤ k_) (_ : k_ ≤ right),
  ...
  have hh1 : ¬lt arr1[mid] arr1[left] := sortAt_sorted (lt := lt) lt_asymm arr left mid (by omega) (by omega)
  have hh2 : ¬lt arr2[right] arr2[left] := sortAt_sorted (lt := lt) lt_asymm arr1 left right (by omega) (by omega)
  have hh_ : ¬lt arr_[right] arr_[mid] := sortAt_sorted (lt := lt) lt_asymm arr2 mid right (by omega) (by omega)
...
include le_trans in
include lt_asymm in
protected theorem hoare.classic.partition_bounds' {arr : Vector α n} {left : Nat} {right : Nat} {hlr : left < right} {hr : right < n} : (hoare.classic (lt := lt) (lt_asymm := lt_asymm) (le_trans := le_trans) arr left right hlr hr).val.j' < (hoare.classic (lt := lt) (lt_asymm := lt_asymm) (le_trans := le_trans) arr left right hlr hr).val.i' := by
...
include le_trans in
include lt_asymm in
protected theorem hoare.classic.permStabilizing' {arr : Vector α n} {left : Nat} {right : Nat} {hlr : left < right} {hr : right < n} : PermStabilizing' left right (hoare.classic (lt := lt) (lt_asymm := lt_asymm) (le_trans := le_trans) arr left right hlr hr).val.arr' arr := by
  apply PermStabilizing'.trans
...
include le_trans in
include lt_asymm in
protected theorem hoare.classic.sorted {arr : Vector α n} {left : Nat} {right : Nat} {hlr : left < right} {hr : right < n} : ∃ (pivot : α), IsPartitioned (lt := lt) left right pivot (hoare.classic  (lt := lt) (lt_asymm := lt_asymm) (le_trans := le_trans) arr left right hlr hr).val :=
  ...
  have hh2 : ¬lt pivot arr_[left] ∧ ¬lt arr_[right] pivot := hoare.classic.median_of_three_sorted (lt := lt) (lt_asymm := lt_asymm) (le_trans := le_trans) hlm hmr hr
  let x : Partition α n := (hoare.classic.loop (lt := lt) left right hr (by omega) pivot  arr_ (left + 1) (right - 1) (by omega) (by omega) (by omega) hh2.left hh2.right).val

  have : IsPartitioned (lt := lt) left right pivot x := by
    have hh1 : IsPartitioned (lt := lt) (left + 1) (right - 1) pivot x := hoare.classic.loop.sorted (lt := lt) (lt_asymm := lt_asymm)

    have : PermStabilizing' (left + 1) (right - 1) x.arr' arr_ := hoare.classic.loop.permStabilizing (lt := lt)
