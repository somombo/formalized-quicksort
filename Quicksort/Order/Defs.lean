...
/- from -/ import Mathlib.Init.Order.Defs
...
-- namespace Or
-- private theorem symm : a ∨ b → b ∨ a := .rec .inr .inl
-- private theorem resolve_left {a b : Prop} (h: a ∨ b) (na : ¬a) : b := h.elim (absurd · na) id
-- private theorem resolve_right {a b : Prop} (h: a ∨ b) (nb : ¬b) : a := h.elim id (absurd · nb)
-- end Or
...
-- def ltByCases (x y : α) {P : Sort v} (h₁ : x < y → P) (h₂ : x = y → P) (h₃ : y < x → P) : P :=
--   if h : x < y then h₁ h
