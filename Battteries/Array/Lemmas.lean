...
theorem swap_self (a : Array α) (i : Nat) (hi : i < a.size /- := by get_elem_tactic -/) : a.swap i i hi hi = a := by
  apply ext
  · simp only [size_swap]
  · intros
...
--   {A a B b C} {i j} {hi_size: i < as.size} {hj_size: j < as.size} (hi : A.length = i) (hj : i + B.length + 1 = j)
--     (eq1 : as.toList = A ++ a :: B ++ b :: C) :
--     (as.swap i j hi_size hj_size).toList = A ++ b :: B ++ a :: C := by
